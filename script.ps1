...
$global:Razones = "`n Puede ser por diferentes razones:`n - El programa de Adobe no está instalado en la ruta por defecto (C:\Archivos de programa).`n - El programa de Adobe no se instaló usando la aplicación Creative Cloud.`n - La versión de Adobe que elegiste no está instalada en tu PC."
...
$global:SinConexion = "`n No es posible conectarse a Internet. Intentando nuevamente en 5s..."
...
function MENU_PRINCIPAL {
    Write-Host "`n`n Simi v1.3 - © Leandro Pérez`n Cambiá el idioma de Adobe sin reinstalar los programas" -Fore DarkGray;
...
function MENU_ADOBE_SPA_A_ENG {
    ...
    Menu 1 8 "`n Adobe $VersionAdobe`n ==========" MENU_ADOBE_SPA_A_ENG "1. After Effects" MENU_AE_SPA_A_ENG "2. Premiere Pro" MENU_PPRO_SPA_A_ENG "3. Audition" MENU_AUDI_SPA_A_ENG "4. InDesign" MENU_IND_SPA_A_ENG "5. Media Encoder" MENU_ME_SPA_A_ENG "6. Photoshop" MENU_PS_SPA_A_ENG "7. Menú principal" MENU_PRINCIPAL "8. Salir" MENU_SALIR "9. Ayuda" MENU_AYUDA
}
...
function MENU_PS_ENG_A_SPA {
    ...
    $RutaInstalacion = "$Env:Programfiles\Adobe\Adobe Photoshop $VersionAdobe\Locales"
    $DestinoDescarga = "$LocaleEsEs.zip"

    if (-not((Test-Path -Path "$RutaInstalacion\$LocaleEsEs" -PathType Container) -or (Test-Path -Path "$RutaInstalacion\$LocaleEsMx" -PathType Container)) ) {
        cd $RutaInstalacion
        ...
        Invoke-WebRequest -Uri $SourceDescarga -OutFile "$RutaInstalacion\$DestinoDescarga"
        Expand-Archive "$LocaleEsEs.zip" -DestinationPath "$RutaInstalacion\$LocaleEsEs" -Force
        Remove-Item "$LocaleEsEs.zip"
        Write-Host $CambioPs -Fore DarkGreen;
    } elseif ((Test-Path -Path "$RutaInstalacion\$LocaleEsEs" -PathType Container) -or (Test-Path -Path "$RutaInstalacion\$LocaleEsMx" -PathType Container)) {
...
function MENU_PS_SPA_A_ENG {
    ...
    if (-not((Test-Path -Path "$RutaInstalacion\$LocaleEnUs" -PathType Container) -or (Test-Path -Path "$RutaInstalacion\$LocaleEnGb" -PathType Container)) ) {
        cd $RutaInstalacion
        # Chequea si hay conexión a Internet
        while (!(Test-Connection -computer leandroperez.art -count 1 -quiet)) {
            Write-Host $SinConexion -Fore Red;
        ...
        Write-Host $Descargando -Fore Yellow;
        Invoke-WebRequest -Uri $SourceDescarga -OutFile "$RutaInstalacion\$DestinoDescarga"
        Expand-Archive "$LocaleEnUs.zip" -DestinationPath "$RutaInstalacion\$LocaleEnUs" -Force
        Remove-Item "$LocaleEnUs.zip"
        Write-Host $CambioPs -Fore DarkGreen;
    } elseif ((Test-Path -Path "$RutaInstalacion\$LocaleEnUs" -PathType Container) -or (Test-Path -Path "$RutaInstalacion\$LocaleEnGb" -PathType Container)) {
        Write-Host $ErrorCambioPs -Fore Red;
    }
